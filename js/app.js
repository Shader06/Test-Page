//подключаем файлы js из папки files с которыми будем работать! 
//Ниже уже подключены файлы которые обычно нужны, лишнее можно удалить!
...
function sliders_bild_callback(params) { }
//для конкретного слайдера подставляем класс (вместо .about__slider подставил .slider)
//И далее меняем настройки под себя (см урок)
...
//Подсказка TIPPY
//можно делать разные подсказки с разными классами
tippy('.tippy', {
...
//RANGE
//поменял .price-filter__slider на название класса из html price__range
//также переделал код, чтобы было как в видеоуроке
...
if (priceSlider) {
	...
	// 	}
	// });
	//поменять настройки под свой проект
	noUiSlider.create(priceSlider, {
		start: [0, 200000],
		connect: true,
...
let scr_items = document.querySelectorAll('._scr-item'); //добавить к объекту для анимации при скроле
//тогда при прокрутке к объектам будет добавляться класс _active и пропадать при уходе объекта (чтобы не пропадал нужно закомментить строку **)
